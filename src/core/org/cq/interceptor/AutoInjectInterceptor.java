...
public class AutoInjectInterceptor implements Interceptor {
	...
	private void inject(Object obj, String prop, String[] values)
			throws Exception {
		String[] props = prop.split("\\.");
		if (props.length == 1) {
			Class<?> type = obj.getClass().getDeclaredField(prop).getType();
			Method set = obj.getClass()
					.getMethod(toMethodName(SET, prop), type);
			if (type == String.class) {
				...
				set.invoke(obj,
						new SimpleDateFormat("YYYY/HH/DD").parse(values[0]));
			} else if (type == Float.class) {
				set.invoke(obj, Float.parseFloat(values[0]));
			} else {
				set = obj.getClass().getMethod(toMethodName(SET, prop),
						String.class);
				set.invoke(obj, values[0]);
			}
		} else {
			Method get = obj.getClass().getMethod(toMethodName(GET, props[0]));
			Method set = obj.getClass().getMethod(toMethodName(SET, props[0]),
					get.getReturnType());
